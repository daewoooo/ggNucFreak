...
plotNucFreq <- function(bamfile = NULL, region = NULL, max.data.points = 100000, base.qual = 25, min.mapq = 0, max.depth = 100, shape = 'point', nucleotide.content = NULL, bin.size = 100000, reverse = FALSE, title = NULL) {
  ## Check user input
  if (!is.null(bamfile)) {
  ...
  pileup <- deepSNV::bam2R(file = bamfile,
  ...
  if (!is.null(nucleotide.content) & methods::is(nucleotide.content, 'character')) {
    ...
    if (all(grepl(letters, pattern='[A,C,G,T]', ignore.case = TRUE))) {
      ...
      plt <- ggplot2::ggplot() +
        ...
        ggplot2::scale_x_continuous(name = 'Genomic position (bp)', expand = c(0, 0), label = comma) +
    ...
    plt <- plt +
      ...
      ggplot2::scale_x_continuous(expand = c(0, 0), label = scales::comma, name = 'Genomic position (bp)') +
